...
function setup_single() {
    ...
    ln -s "$(realpath -L ../$1)" "$runner_dir/$(basename $1)"

    echo "$runner_dir"
}

function run_pipelineComplete() {
    local working_dir="$(setup_single timing/pipeline_complete/measureTime_pipelineComplete.py)"
    cd "$working_dir"
    ...
    taskset -c $1 ./../measureTime_pipelineComplete.py $2 $3
...
function run_pipelineMatchingSteps() {
    local working_dir="$(setup_single timing/pipeline_steps/measureTime_matchingSteps.py)"
    cd "$working_dir"
    ...
    taskset -c $1 ./../measureTime_matchingSteps.py $2 $3
...
function run_pipelineSteps() {
    local working_dir="$(setup_single timing/pipeline_steps/measureTime_pipelineSteps.py)"
    cd "$working_dir"
    ...
    taskset -c $1 ./../measureTime_pipelineSteps.py $2 $3
