...
int main() {
...
#ifdef WITH_OPT
    std::string pathApped("after_optimizations/") distResPath += pathAppend;
#endif
    ...
    std::cout << "\rRunning pipeline finished.                        "
              << std::endl;
    ...
    for (auto it = pipelinedImages.begin(); it != pipelinedImages.end();
        ...
        std::cout << "\r(" << i << ") Processing " << fileName1Short
                  << "...                        " << std::flush;
