 ...
 * indices (rows).
 *
 * @param[in] input: A 2-dimensional NdArray of doubles for which the elements
 * are checked.
 ...
 * @returns A tuple of size 3, where the first value is a 1-dimensional NdArray
 * of doubles containing the x-axis indices (columns), the second value is a
 * 1-dimensional NdArray of doubles containing the y-axis indices (rows), and
 * the last value denotes the size of these two arrays.
 */
 ...
 * contains the rotated input matrix.
 */
template <typename T>
nc::NdArray<T> rotateMat(nc::NdArray<T> rotMat, double angle, double x,
    ...
    cv::Mat rotationMatrix =
        cv::getRotationMatrix2D(cv::Point2f(y, x), angle, 1.0);
 ...
 *
 * @param[inout] img: A reference to a 2-dimensional NdArray of uint8_t values
 * containing the image that we want to translate. After computation this
 * reference will point to the translated matrix.
 * @param[in] mask: A 2-dimensional NdArray of uint8_t values containing the
 * mask on which we compute the fitting and that we want to translate.
 * @param[in] width: An integer denoting the width (#columns) of the input
 * matrices.
 * @param[in] height: An integer denoting the height (#rows) of the input
 * matrices.
 * @returns A 2-dimensional NdArray of doubles containing the translated mask
 * matrix.
 */
