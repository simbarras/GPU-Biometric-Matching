...
std::vector<nc::NdArray<double>> detect_valleys(nc::NdArray<double> image,
    ...
    std::pair<nc::NdArray<double>, nc::NdArray<double>> res =
        nc::meshgrid(window, window);
    ...
    cv::filter2D(imageOCV, image_G1_0OCV, -1, G1_0OCV, cv::Point(-1, -1), (0.0),
                 cv::BORDER_REPLICATE);
    cv::filter2D(imageOCV, image_G2_0OCV, -1, G2_0OCV, cv::Point(-1, -1), (0.0),
                 cv::BORDER_REPLICATE);
    cv::filter2D(imageOCV, image_G1_90OCV, -1, G1_90OCV, cv::Point(-1, -1),
                 (0.0), cv::BORDER_REPLICATE);
    cv::filter2D(imageOCV, image_G2_90OCV, -1, G2_90OCV, cv::Point(-1, -1),
                 (0.0), cv::BORDER_REPLICATE);
    cv::filter2D(imageOCV, fxyOCV, -1, hxyOCV, cv::Point(-1, -1), (0.0),
                 cv::BORDER_REPLICATE);
    ...
    nc::NdArray<double> image_g1_45 =
        0.5 * sqrt(2) * (image_g1_0 + image_g1_90);
    nc::NdArray<double> image_g1_m45 =
        0.5 * sqrt(2) * (image_g1_0 - image_g1_90);

    nc::NdArray<double> image_g2_45 =
        0.5 * image_g2_0 + fxy + 0.5 * image_g2_90;
    nc::NdArray<double> image_g2_m45 =
        0.5 * image_g2_0 - fxy + 0.5 * image_g2_90;

    nc::NdArray<double> r1 =
        (image_g2_0 /
         (nc::sqrt((nc::power((1.0 + nc::power(image_g1_0, 2)), 3))))) *
        mask;
    nc::NdArray<double> r2 =
        (image_g2_90 /
         (nc::sqrt((nc::power((1.0 + nc::power(image_g1_90, 2)), 3))))) *
        mask;
    nc::NdArray<double> r3 =
        (image_g2_45 /
         (nc::sqrt((nc::power((1.0 + nc::power(image_g1_45, 2)), 3))))) *
        mask;
    nc::NdArray<double> r4 =
        (image_g2_m45 /
         (nc::sqrt((nc::power((1.0 + nc::power(image_g1_m45, 2)), 3))))) *
        mask;
...
nc::NdArray<double> _prob_1d(nc::NdArray<double> a, int width) {
    ...
    for (size_t i = 0; i < starts.size() || i < ends.size(); i++) {
        ...
        z(0, start + maximum) =
            a(0, start + maximum) * static_cast<double>(end - start);
    }

    return z;
}

std::vector<std::tuple<int, int>> diag_indices(int nth_diag, int width,
                                               int height) {
...
nc::NdArray<double> diag_elems(nc::NdArray<double> mat, int nth_diag, int width,
                               int height) {
    assert(nth_diag < width && nth_diag > -height);
...
nc::NdArray<double>
eval_vein_probabilities(std::vector<nc::NdArray<double>> input_matrices,
    ...
    for (int i = 0; i < height; i++) {
        V.put(i, cSlicerV,
              (V(i, cSlicerV) +
               _prob_1d_opt(input_matrices.at(0)(i, cSlicerV), width)));
    }
    ...
    for (int j = 0; j < width; j++) {
        V.put(rSlicerV, j,
              (nc::flatten(V(rSlicerV, j)) +
               (_prob_1d_opt(nc::flatten(input_matrices.at(1)(rSlicerV, j)),
                             height)))
                  .reshape(height, 1));
    }
    ...
    for (int index = -height + 1; index < width; index++) {
        std::vector<std::tuple<int, int>> indicesDiag =
            diag_indices(index, width, height);

        nc::NdArray<double> Vadd = _prob_1d_opt(
            diag_elems(curv, index, width, height), indicesDiag.size());
...
nc::NdArray<double> _connect_1d(nc::NdArray<double> a, int width) {
    ...
    nc::NdArray<double> max1 = nc::amax(
        (nc::stack({a(0, s1), a(0, s2)}, nc::Axis::ROW)), nc::Axis::ROW);
    nc::NdArray<double> max2 = nc::amax(
        (nc::stack({a(0, s3), a(0, s4)}, nc::Axis::ROW)), nc::Axis::ROW);

    return nc::amin(nc::stack({max1, max2}, nc::Axis::ROW), nc::Axis::ROW);
}

std::vector<nc::NdArray<double>> connect_centers(nc::NdArray<double> V,
                                                 int width, int height) {
    ...
    for (int i = 0; i < width; i++) {
        a2.put(rSlicera2, i,
               (_connect_1d_opt(nc::flatten(V(rSlicerV, i)), height)));
    }
    ...
    for (int index = -height + 5; index < width - 4; index++) {
        ...
        nc::NdArray<double> in =
            nc::hstack({border,
                        _connect_1d_opt(diag_elems(V, index, width, height),
                                        indicesDiag.size()),
                        border});
    ...
    for (int index = -height + 5; index < width - 4; index++) {
        ...
        nc::NdArray<double> in =
            nc::hstack({border,
                        _connect_1d_opt(diag_elems(Vud, index, width, height),
                                        indicesDiag.size()),
                        border});
...
nc::NdArray<bool> binarise(nc::NdArray<double> G) {
    ...
    if (Gnew.size() != 0) {
        ...
        std::cout << "Something went terribly wrong, we did not find any veins!"
                  << std::endl;
        exit(1);
    }
...
nc::NdArray<bool> maximum_curvature(nc::NdArray<uint8_t> image,
                                    nc::NdArray<double> mask, int width,
    ...
    std::vector<nc::NdArray<double>> kappa =
        detect_valleys(finger_image, mask, sigma, width, height);
