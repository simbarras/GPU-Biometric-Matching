...
int main(int argc, char **argv) {
    ...
    std::ofstream prealignmentFile(timePath + prealigmentPath + time +
                                   csvEnding);
    ...
    std::cout << "Warm-up finished." << std::endl
              << "Timing for pipeline steps done for:" << std::endl;
    ...
    for (auto it = files.begin(); it != files.end(); it++, j++) {
        ...
        std::cout << "(" << j << ") Processing " << fileNameShort
                  << "...                        " << std::endl;
        ...
        for (int i = 0; i < 15; i++) {
            std::chrono::high_resolution_clock::time_point timeMCStart =
                std::chrono::high_resolution_clock::now();
            nc::NdArray<bool> veins =
                maximum_curvature(image, res, width, height);
    ...
    for (size_t i = min_image_no; i < max_image_no; i++) {
        ...
        timesPerImage = timesMCurv.at(i - min_image_no);
        for (auto it = timesPerImage.begin(); it != timesPerImage.end(); it++) {
