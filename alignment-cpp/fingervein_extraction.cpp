...
size_t register_fingervein_single(const int width, const int height,
                                  ...
                                  uint8_t **modelOut, uint8_t *imageIn) {

    // Creates uint8_t NdArray from uint8_t pointer
    nc::NdArray<uint8_t> image =
        nc::NdArray<uint8_t>(imageIn, height, width, nc::PointerPolicy::COPY);

    // Runs the pipeline with the given inputs and the generated image NdArray
    nc::NdArray<bool> res =
        run_pipeline(width, height, camera_perspective, &image);
    ...
    int n =
        cv::getOptimalDFTSize(veinsOCV.cols); // on the border add zero values
    cv::copyMakeBorder(veinsOCV, padded, 0, m - veinsOCV.rows, 0,
                       n - veinsOCV.cols, cv::BORDER_CONSTANT,
                       cv::Scalar::all(0));
    ...
    uint32_t numNonZero =
        static_cast<u_int32_t>(complexI.at<std::complex<float>>(0, 0).real());
...
size_t register_fingerveins(const int width, const int height,
                            uint8_t **modelOut, uint8_t *imageIn1,
                            uint8_t *imageIn2) {

    // Creates uint8_t NdArray from uint8_t pointer
    nc::NdArray<uint8_t> image1 =
        nc::NdArray<uint8_t>(imageIn1, height, width, nc::PointerPolicy::COPY);
    nc::NdArray<uint8_t> image2 =
        nc::NdArray<uint8_t>(imageIn2, height, width, nc::PointerPolicy::COPY);
    ...
    int n =
        cv::getOptimalDFTSize(veins1OCV.cols); // on the border add zero values
    cv::copyMakeBorder(veins1OCV, padded1, 0, m - veins1OCV.rows, 0,
                       n - veins1OCV.cols, cv::BORDER_CONSTANT,
                       cv::Scalar::all(0));
    cv::copyMakeBorder(veins2OCV, padded2, 0, m - veins2OCV.rows, 0,
                       n - veins2OCV.cols, cv::BORDER_CONSTANT,
                       cv::Scalar::all(0));

    cv::Mat planes1[] = {cv::Mat_<float>(padded1),
                         cv::Mat::zeros(padded1.size(), CV_32F)};
    cv::Mat planes2[] = {cv::Mat_<float>(padded2),
                         cv::Mat::zeros(padded2.size(), CV_32F)};
    cv::Mat complexI1;
    cv::Mat complexI2;
    cv::merge(planes1, 2,
              complexI1); // Add to the expanded another plane with zeros
    ...
    uint32_t numNonZero1 =
        static_cast<u_int32_t>(complexI1.at<std::complex<float>>(0, 0).real());
    uint32_t numNonZero2 =
        static_cast<u_int32_t>(complexI2.at<std::complex<float>>(0, 0).real());
...
void freeProbeCache(struct probeCache *cache) { delete cache; }

bool compare_model_with_input_single(const int width, const int height,
                                     const int camera_perspective,
                                     uint8_t *imageIn, uint8_t *modelIn,
                                     size_t modelSize,
    ...
    if (!(probeC->alreadyCached)) {
        ...
        int n = cv::getOptimalDFTSize(
            probeOCV.cols); // on the border add zero values
        cv::copyMakeBorder(probeOCV, padded, 0, m - probeOCV.rows, 0,
                           n - probeOCV.cols, cv::BORDER_CONSTANT,
                           cv::Scalar::all(0));
        ...
        // Counts the number of non-zero values in vein image
        uint32_t numNonZero = static_cast<u_int32_t>(
            complexI.at<std::complex<float>>(0, 0).real());
    ...
    // Computes the distance between the images using their Fourier transform
    // and the number of non-zeros values
    double dist =
        compute_miura_distance_opt(model, probeC->cachedProbe1, modelNonZeros,
                                   probeC->cachedProbe1NumNonZeros);

    return dist < THRESHOLD;
}

bool compare_model_with_input(const int width, const int height,
                              const double tau, uint8_t *imageIn1,
                              uint8_t *imageIn2, u_int8_t *modelIn,
                              size_t modelSize, struct probeCache *probeC) {
    ...
    if (!(probeC->alreadyCached)) {
        ...
        nc::NdArray<uint8_t> image1 = nc::NdArray<uint8_t>(
            imageIn1, height, width, nc::PointerPolicy::COPY);
        nc::NdArray<uint8_t> image2 = nc::NdArray<uint8_t>(
            imageIn2, height, width, nc::PointerPolicy::COPY);
        ...
        int n = cv::getOptimalDFTSize(
            probe1OCV.cols); // on the border add zero values
        cv::copyMakeBorder(probe1OCV, padded1, 0, m - probe1OCV.rows, 0,
                           n - probe1OCV.cols, cv::BORDER_CONSTANT,
                           cv::Scalar::all(0));
        cv::copyMakeBorder(probe2OCV, padded2, 0, m - probe2OCV.rows, 0,
                           n - probe2OCV.cols, cv::BORDER_CONSTANT,
                           cv::Scalar::all(0));
        ...
        // Counts the number of non-zero values in the vein images
        uint32_t numNonZero1 = static_cast<u_int32_t>(
            complexI1.at<std::complex<float>>(0, 0).real());
        uint32_t numNonZero2 = static_cast<u_int32_t>(
            complexI2.at<std::complex<float>>(0, 0).real());
    ...
    // Extracts the memory region containing the number of non-zeros values for
    // both images
    ...
    double dist1 =
        compute_miura_distance_opt(model1, probeC->cachedProbe1, modelNonZeros1,
                                   probeC->cachedProbe1NumNonZeros);
    double dist2 =
        compute_miura_distance_opt(model2, probeC->cachedProbe2, modelNonZeros2,
                                   probeC->cachedProbe2NumNonZeros);
...
void free_model(uint8_t *model) { delete model; }
