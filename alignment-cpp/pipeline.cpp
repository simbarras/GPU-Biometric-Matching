...
void saveToPNG(std::string saveIntermediateSteps_path, nc::NdArray<int> image,
...
nc::NdArray<bool> run_pipeline(const int width, const int height,
    ...
    nc::NdArray<uint8_t> mask =
        edge_mask_extraction_opt(img, camera_persp, width, height);
...
// Saves masks if needed
    ...
    saveToPNG(pathMain + path1 + time + maskSave, mask.astype<int>(), width,
              height);
#endif
...
// Saves masks if needed
    ...
    saveToPNG(pathMain + path2 + time + imageSave, img.astype<int>(), width,
              height);
#endif
...
// Saves masks if needed
    ...
    saveToPNG(pathMain + path3 + time + imageSave, veins.astype<int>(), width,
              height);
#endif
