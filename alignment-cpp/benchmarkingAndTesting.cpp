...
int main() {
    ...
    uint8_t *imageIn = readpng_file_to_array(
        "../dataset/0_left_index_1_cam1.png", width, height);
...
#if defined(BENCHMARK_PIPELINE) || defined(BENCHMARK_PIPELINE_STEPS) ||        \
    defined(BENCHMARK_COMPARISON)
    ...
    pipelineTiming
        << ", Edge_Mask_Extraction_Time, Translation_Alignment_Time, "
           "Maximum_Curvature_Time, Miura_Matching_Time";
#endif
    ...
    for (auto it = files.begin(); it != files.end(); it++) {
        ...
        image = nc::NdArray<uint8_t>(imageIn, height, width,
...
// Benchmark for single finger pipeline run
        ...
        for (int i = 0; i < 15; i++) {
            std::chrono::high_resolution_clock::time_point timePipStart =
                std::chrono::high_resolution_clock::now();
            model = run_pipeline(width, height, (camPersp1 - '0'), &image);
            std::chrono::high_resolution_clock::time_point timePipEnd =
                std::chrono::high_resolution_clock::now();

            std::chrono::duration<double> time_spanPip =
                std::chrono::duration_cast<std::chrono::duration<double>>(
                    timePipEnd - timePipStart);
            timeSpanPip += time_spanPip;

            // either save time spans in file or do some statistical computation
            // here and output result

            // std::cout << "Pipeline for " << fileName1 << " took " <<
            // time_spanPip.count() << std::endl;
        }
        ...
        for (int i = 0; i < 15; i++) {
            std::chrono::high_resolution_clock::time_point timeMEStart =
                std::chrono::high_resolution_clock::now();
            nc::NdArray<uint8_t> mask =
                edge_mask_extraction(image, (camPersp1 - '0'), width, height);
            std::chrono::high_resolution_clock::time_point timeMEEnd =
                std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> time_spanME =
                std::chrono::duration_cast<std::chrono::duration<double>>(
                    timeMEEnd - timeMEStart);

            std::chrono::high_resolution_clock::time_point timeTAStart =
                std::chrono::high_resolution_clock::now();
            std::tuple<nc::NdArray<uint8_t>, nc::NdArray<double>> res =
                translation_alignment(image, mask, width, height);
            std::chrono::high_resolution_clock::time_point timeTAEnd =
                std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> time_spanTA =
                std::chrono::duration_cast<std::chrono::duration<double>>(
                    timeTAEnd - timeTAStart);

            std::chrono::high_resolution_clock::time_point timeMCStart =
                std::chrono::high_resolution_clock::now();
            nc::NdArray<bool> veins = maximum_curvature(
                std::get<0>(res), std::get<1>(res), width, height);
            std::chrono::high_resolution_clock::time_point timeMCEnd =
                std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> time_spanMC =
                std::chrono::duration_cast<std::chrono::duration<double>>(
                    timeMCEnd - timeMCStart);
        ...
        if (pipelineTiming.is_open()) {
            pipelineTiming << (timeSpanME / 15.).count() << ", "
                           << (timeSpanTA / 15.).count() << ", "
                           << (timeSpanMC / 15.).count() << ", ";
        }
#endif
        ...
        for (auto it2 = it; it2 != files.end(); it2++) {
            ...
            if (camPersp1 == camPersp2) {
                ...
                if (fileIdentifier1.compare(fileIdentifier2) == 0) {
                    ...
                    // write distance results in different file than for
                    // different fingers
                    sameDist << dist
                             << ", "; // << " (" << fileName2Short << "), ";
                    continue;
                ...
                // write distance results in different file than for same
                // fingers
...
#ifdef BENCHMARK_COMPARISON
        if (pipelineTiming.is_open()) {
            pipelineTiming << (timeSpanD / numPics_iterated).count() << ", "
                           << std::endl;
        }
#endif
