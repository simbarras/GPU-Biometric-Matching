...
std::tuple<int, int> miura_score(nc::NdArray<bool> model,
                                 nc::NdArray<bool> probe, int width, int height,
                                 int ch, int cw) {
    ...
    cv::filter2D(probeOCV, NcOCV, -1, kernelOCV, cv::Point(-1, -1), (0.0),
                 cv::BORDER_REPLICATE);
    ...
    std::tuple<int, int> unrav =
        unravel_index(N_c.argmax()(0, 0), widthNc, heightNc);
    ...
    double R_c = N_c(t0, s0) / ((nc::count_nonzero(crop_model)(0, 0)) +
    (nc::count_nonzero(probe(rSlice2, cSlice2))(0, 0)));
    */
    return {t0, s0};
}

nc::NdArray<bool> miura_matching(nc::NdArray<bool> image,
                                 const nc::NdArray<bool> model, int width,
                                 int height) {
