 ...
 * traversing up and down. It either finds the index of the maximum value
 * below threshold or the index of the first element exceeding this
 * threshold. This is an optimized version to the one below.
 *
 * @param[in] arr: A 1-dimensional NdArray of doubles that needs to be
 * traversed.
 ...
 * (default = 4)
 * @returns A tuple of indices pointing to the maximum elements in the NdArray.
 * These elements are either the maximum element of all traversed elements if
 * all values were below the threshold, or it refers to the first encountered
 * value that is above the threshold. In that way it finds the edges of an
 * image.
 */
std::tuple<int, int> max_thresh_opt(nc::NdArray<double> arr, int start = 130,
                                    int threshold = 4);
 ...
 */
std::array<int, 3> edge_points(nc::NdArray<double> img, int x_1, int f_1 = 130,
                               int threshold = 4);

/**
 * This function takes as input an image of a finger stored in a NdArray and
 * computes a mask for the finger. This is an optimized version of the
 ...
 */
nc::NdArray<uint8_t>
edge_mask_extraction_opt(const nc::NdArray<uint8_t> img, int camera_persp,
 ...
 */
nc::NdArray<uint8_t>
edge_mask_extraction(const nc::NdArray<uint8_t> img, int camera_persp,
                     int width, int height,
                     std::tuple<int, int> roi1 = {35, 355},
                     std::tuple<int, int> roi2 = {55, 360});
